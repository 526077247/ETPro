...
public class ArtToolsWindow: EditorWindow
{
	...
	private static long ThreadFind(ThreadPars par)
	{
		...
		if (par != null)
		{
			...
			{
				ret += LoadFilesByThread(par.ChildDataList[i], par.DataPathList[i], 2);
			}
		}
	...
	private static long LoadFilesThread(Data root)
	{
		...
		long[] dataSize = new long[root.childs.Count];
		long totalSize = 0;
		...
		var i = 0;
		for (int j = 0; j < root.childs.Count; j++)
		{
			var data = root.childs[j];
			foreach (var path in Directory.GetFiles(data.assetPath))
			{
				var content = GetGUIContent(path);
				if (content != null)
				{
					Data child = new Data();
					child.indent = data.indent + 1;
					child.content = content;
					child.assetPath = path;
					child.isIllegalImage = ValidateImage(path);
					child.fileMemorySize = MemorySize(path);
			...
			foreach (var path in Directory.GetDirectories(data.assetPath))
			{
				Data childDir = new Data();
				data.childs.Add(childDir);
				...
				threadParses[index].ChildIndex = j;
				i++;
		...
		_updateDelegate = delegate
		{
			...
			EditorUtility.DisplayProgressBar("匹配资源中", string.Format("进度：{0}", finishedCount),
				(float) finishedCount / ThreadCount);
			...
			if (finishedCount >= finishedState)
			{
				...
				for (int j = 0; j < ThreadCount; j++)
				{
					var size = tRun[j].EndInvoke(results[j]);
					dataSize[threadParses[j].ChildIndex] += size;
					totalSize += size;
				}
	...
	private static void initFileData()
	{
		...
		var child = new Data();
		child.indent = 1;
		child.content = GetGUIContent(path, "Assets");
		child.assetPath = path;
		child.isIllegalImage = false;
		child.fileMemorySize = 0;
		child.isCreateContent = true;
		data.childs.Add(child);
		...
		{
			path = $"Modules/{directories[i].Name}/AssetsPackage";
			if (!Directory.Exists(path)) continue;
			child = new Data();
			child.indent = 1;
			child.content = GetGUIContent(path, "Modules/" + directories[i].Name);
			child.assetPath = path;
			child.isIllegalImage = false;
			child.fileMemorySize = 0;
			child.isCreateContent = true;
		...
		for (int i = 0; i < directories.Length; i++)
		{
			path = $"Packages/{directories[i].Name}/AssetsPackage";
			if (!Directory.Exists(path)) continue;
			child = new Data();
			child.indent = 1;
			child.content = GetGUIContent(path, "Packages/" + directories[i].Name);
			child.assetPath = path;
			child.isIllegalImage = false;
			child.fileMemorySize = 0;
			child.isCreateContent = true;
		...
		data.indent = 0;
		data.content = new GUIContent("AssetsPackage");
		data.assetPath = "";
		data.isIllegalImage = false;
		data.fileMemorySize = 0;
		data.isCreateContent = true;
		LoadFilesThread(data);
	}
 ...
 */
		...
		/*      
		   ...
		   */
		ResourceCheckTool.FindThread(assetPath);
	...
	private static GUIContent GetGUIContent(string path, string name = null)
	{
		path = path.Replace("Modules", "Packages");
		...
		if (asset)
		{
			...
			return new GUIContent(name, AssetDatabase.GetCachedIcon(path));
		}
